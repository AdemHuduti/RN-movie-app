...
class AllPopularTvShows extends Component {
  ...
  getPopularTvShows() {
    const { showAllTvShows } = this.props;
    if (!showAllTvShows.length) {
  ...
  loadAllPopularTvShows = (data) => this.props.dispatch({ type: 'GET_ALL_POPULAR_TV_SHOWS', data });
  ...
  renderPopularTvShows() {
    const { showAllTvShows } = this.props;
    return showAllTvShows.map((show, i) => {
      return (
        <View key={i}>
          <Image
            resizeMode="cover"
...
const mapStateToProps = (state) => ({
  showAllTvShows: state.showAllTvShows,
